...
func NewCreateProductHandler(repo product.Repository, outbox outbox.Outbox, txManager persistence.TxManager, eventFactory event.ProductEventFactory) CreateProductCommandHandler {
...
func (h *createProductHandler) Handle(ctx context.Context, cmd CreateProductCommand) (*product.Product, error) {
	...
	result, err := h.txManager.WithTransaction(ctx, func(txCtx context.Context) (any, error) {
		...
		msg, err := h.eventFactory.NewProductCreatedOutboxMessage(txCtx, p)
		if err != nil {
			return nil, err
		}
