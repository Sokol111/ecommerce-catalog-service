...
func TestAttribute_CreateAndGet(t *testing.T) {
	...
	// 1. Create attribute via API
	createReq := &catalogv1.CreateAttributeRequest{
		Name:    "Color",
		Slug:    "color-e2e-test-" + uuid.New().String()[:8],
		Type:    catalogv1.CreateAttributeRequestTypeSingle,
		Enabled: true,
		Options: []catalogv1.AttributeOptionInput{
			{Name: "Red", Slug: "red", SortOrder: catalogv1.NewOptInt(1)},
			{Name: "Blue", Slug: "blue", SortOrder: catalogv1.NewOptInt(2)},
		},
	}

	createResp, err := testClient.CreateAttribute(ctx, createReq)
	require.NoError(t, err)
	require.IsType(t, &catalogv1.AttributeResponse{}, createResp)

	created := createResp.(*catalogv1.AttributeResponse)
	assert.NotEmpty(t, created.ID)
	assert.Equal(t, "Color", created.Name)
	assert.Equal(t, catalogv1.AttributeResponseTypeSingle, created.Type)
	...
	getResp, err := testClient.GetAttributeById(ctx, catalogv1.GetAttributeByIdParams{
		ID: createdID,
	})
	require.NoError(t, err)
	require.IsType(t, &catalogv1.AttributeResponse{}, getResp)

	fetched := getResp.(*catalogv1.AttributeResponse)
...
func TestAttribute_Update(t *testing.T) {
	...
	updateReq := &catalogv1.UpdateAttributeRequest{
		ID:      createdID,
		Name:    "Size Updated",
		Enabled: true,
		Version: created.Version,
		Options: []catalogv1.AttributeOptionInput{
			{Name: "Small", Slug: "small", SortOrder: catalogv1.NewOptInt(1)},
			{Name: "Large", Slug: "large", SortOrder: catalogv1.NewOptInt(2)},
		},
	}

	updateResp, err := testClient.UpdateAttribute(ctx, updateReq)
	require.NoError(t, err)
	require.IsType(t, &catalogv1.AttributeResponse{}, updateResp)

	updated := updateResp.(*catalogv1.AttributeResponse)
...
func TestAttribute_List(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		createReq := &catalogv1.CreateAttributeRequest{
			Name:    "ListTest" + string(rune('0'+i)),
			Slug:    "list-test-" + uuid.New().String()[:8],
			Type:    catalogv1.CreateAttributeRequestTypeText,
			Enabled: true,
		}
	...
	// 2. Get list
	listResp, err := testClient.GetAttributeList(ctx, catalogv1.GetAttributeListParams{
		Page: 1,
		Size: 10,
	})
	require.NoError(t, err)

	list, ok := listResp.(*catalogv1.AttributeListResponse)
	require.True(t, ok, "expected *catalogv1.AttributeListResponse, got %T", listResp)
...
func TestAttribute_ListWithFilters(t *testing.T) {
	...
	// 1. Create attributes with different types and enabled status
	enabledSingle := &catalogv1.CreateAttributeRequest{
		Name:    "FilterTest Enabled Single",
		Slug:    "filter-enabled-single-" + uniqueSuffix,
		Type:    catalogv1.CreateAttributeRequestTypeSingle,
		Enabled: true,
		Options: []catalogv1.AttributeOptionInput{
			{Name: "Option1", Slug: "opt1", SortOrder: catalogv1.NewOptInt(1)},
		},
	}
	_, err := testAttributeClient.CreateAttribute(ctx, connect.NewRequest(enabledSingle))
	require.NoError(t, err)

	disabledBoolean := &catalogv1.CreateAttributeRequest{
		Name:    "FilterTest Disabled Boolean",
		Slug:    "filter-disabled-bool-" + uniqueSuffix,
		Type:    catalogv1.CreateAttributeRequestTypeBoolean,
		Enabled: false,
	}
	_, err = testAttributeClient.CreateAttribute(ctx, connect.NewRequest(disabledBoolean))
	require.NoError(t, err)
	...
	require.NoError(t, err)
	list := listResp.(*catalogv1.AttributeListResponse)
	...
	listResp, err = testAttributeClient.GetAttributeList(ctx, connect.NewRequest(&catalogv1.GetAttributeListRequest{
		Page: 1,
		Size: 100,
		Type: catalogv1.NewOptGetAttributeListType(catalogv1.GetAttributeListTypeBoolean),
	}))
	require.NoError(t, err)
	list = listResp.(*catalogv1.AttributeListResponse)

	for _, item := range list.Items {
		assert.Equal(t, catalogv1.AttributeResponseTypeBoolean, item.Type, "expected all items to be boolean type")
	}
	...
	listResp, err = testClient.GetAttributeList(ctx, catalogv1.GetAttributeListParams{
		Page:  1,
		Size:  10,
		Sort:  catalogv1.NewOptGetAttributeListSort(catalogv1.GetAttributeListSortName),
		Order: catalogv1.NewOptGetAttributeListOrder(catalogv1.GetAttributeListOrderDesc),
	})
	require.NoError(t, err)
	list = listResp.(*catalogv1.AttributeListResponse)
...
func TestAttribute_NotFound(t *testing.T) {
	...
	resp, err := testClient.GetAttributeById(ctx, catalogv1.GetAttributeByIdParams{
		ID: uuid.New(),
	})
	require.NoError(t, err)

	// Should return NotFound response type
	_, ok := resp.(*catalogv1.GetAttributeByIdNotFound)
	assert.True(t, ok, "expected *catalogv1.GetAttributeByIdNotFound, got %T", resp)
}
...
func TestAttribute_DuplicateSlug(t *testing.T) {
	...
	// 2. Try to create with same slug
	duplicateReq := &catalogv1.CreateAttributeRequest{
		Name:    "Another Attr",
	...
	_, ok := resp.(*catalogv1.CreateAttributeConflict)
	assert.True(t, ok, "expected *catalogv1.CreateAttributeConflict, got %T", resp)
}
...
func TestAttribute_UpdateNotFound(t *testing.T) {
	...
	// Should return NotFound response type
	_, ok := resp.(*catalogv1.UpdateAttributeNotFound)
	assert.True(t, ok, "expected *catalogv1.UpdateAttributeNotFound, got %T", resp)
}
...
func TestAttribute_UpdateVersionMismatch(t *testing.T) {
	...
	_, ok := resp.(*catalogv1.UpdateAttributePreconditionFailed)
	assert.True(t, ok, "expected *catalogv1.UpdateAttributePreconditionFailed, got %T", resp)
}
